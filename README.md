...
> État décrit : implémentation présente dans ce dossier au 6 juillet 2026
...
- un runtime descriptif nommé « Hotel IA ».
...
flowchart LR
    ...
    P --> UI["Modules métier et dashboard"]
    P --> LS["localStorage"]
    LS --> UI
    P --> RT["Hotel Runtime déterministe"]
    RT --> HI["Vue Hotel IA"]
...
1. `script.js`
2. `dd.module.js`
3. `overview.module.js`
4. `inventory.module.js`
5. `todo.module.js`
6. `groups.module.js`
7. `hotel.structure.js`
8. `hotel.model.js`
9. `hotel.entities.js`
10. `hotel.state.js`
11. `hotel.adapters.js`
12. `hotel.runtime.js`
13. `plan.module.js`
14. `hotel-ia.module.js`
...
| `window.HOTEL_RUNTIME` | construction de l’état hôtel unifié |
...
| `hotel.runtime.js` | calcul des snapshots et signaux descriptifs |
| `hotel-ia.module.js` | affichage JSON du runtime |
...
- groupes potentiellement non éclatés ;
...
La vue « Hotel IA » affiche le runtime sous forme de cartes et de JSON : snapshot, sources, signaux, réservations, groupes, préférences, modèle et tranches de modules.

Le nom décrit une **couche de préparation pour une IA future**. Dans l’implémentation actuelle :

- aucun modèle de langage n’est appelé ;
- aucun prompt n’est envoyé ;
- aucune prédiction n’est produite ;
- aucune décision n’est exécutée ;
- aucun historique d’état hôtel n’est conservé.
...
| `nonSplit` | vrai si au moins une ligne porte `NB_RESA > 1` |
...
Il adapte ensuite chaque source vers les entités communes, calcule les signaux et génère le snapshot.
...
- nombre d’adultes ;
- alerte `NON ÉCLATÉ` si une ligne agrégée représente plusieurs réservations.
...
- Un groupe est « non éclaté » si au moins une ligne a `NB_RESA > 1`.
...
- Hotel IA : reconstruction descriptive, sans raisonnement IA ;
...
| IA | architecture LLM prévue | aucun LLM | présenter Hotel IA comme runtime préparatoire |
...
7. Ajouter des tests de non-régression sur les compteurs dashboard/runtime.
8. Découper `script.js` par domaine.
9. consolider `styles.css` sans modifier l’interface fonctionnelle.
10. Documenter et versionner le contrat `/api/github` ou le remplacer.
...
| Éclaté | groupe représenté par des réservations unitaires ; `NB_RESA = 1` par ligne |
