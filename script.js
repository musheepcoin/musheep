...
function buildKeywordRegex(list, mode = 'word'){
  ...
  function saveFolsSnapshot(_rawCsv, rows){
    ...
    saveFolsSnapshotsMap(snapshots);
...
const sofaCountToday = todayGroup
  ...
  function processCsvText(csvText, options = {}){
    ...
    if (saveSnapshot) {
  try {
    saveFolsSnapshot(csvText, rows);
  } catch (e) {
    console.warn("Snapshot ignoré:", e);
  }
}
    LAST_FOLS_ROWS = rows;
...
})();async function handleIndivFile(file) {
  try {
    const text = await file.text();

    const cleanText = text.replace(/^\uFEFF/, "");

    processCsvText(cleanText, { saveSnapshot: false });
    ...
    if (typeof showToast === "function") {
      showToast("Portefeuille chargé avec succès", "success");
    }

  } catch (e) {
    console.error("FOLS import error:", e);

    if (typeof showToast === "function") {
      showToast("Erreur import FOLS", "error");
    }
  }
}
